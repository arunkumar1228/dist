...
@Component({
  selector: 'app-sap-programming',
  templateUrl: './sap-programming.component.html',
  styleUrls: ['./sap-programming.component.scss']
})
export class SapProgrammingComponent implements OnInit{
  currentUrl: string;
  serviceArray = [];
  caseStudyArray = [];
  caseStudyArrayList = [];
  ...
  constructor(private httpClient: HttpClient,private titleService: Title,private metaService: Meta,private location:Location,private renderer: Renderer2,private router: Router,private caseStudyService:CaseStudyService){
    this.titleService.setTitle('SAP Programming Experts for Robust Solutions');
    this.metaService.addTag({ name: 'description', content:"Unlock the full potential of SAP with our expert programming services. Tailored solutions for robust SAP programming, driving efficiency and innovation in your enterprise." });
  
  }
  
  ngOnInit(): void {
    
    this.caseStudyList('SapProgramming');
