...
@Component({
  selector: 'app-itom',
  templateUrl: './itom.component.html',
  styleUrls: ['./itom.component.scss']
})
export class ItomComponent implements OnInit {
  currentUrl: string;
  serviceArray = [];
  constructor(private httpClient: HttpClient,private titleService: Title,private metaService: Meta,private location:Location,private renderer: Renderer2,private router: Router) {
    this.titleService.setTitle('Enhance Operations with ServiceNow ITOM Solutions');
    this.metaService.addTag({ name: 'description', content:"Optimize and streamline operations using ServiceNow ITOM. Our solutions bring efficiency, visibility, and control. Explore the power of ITOM for superior business performance." });
  ...
  blogList() {
    this.httpClient
      .get('https://qbrainx.com/qbrainx-web/v1/Common-Service/byServiceName?serviceName=itom', {
