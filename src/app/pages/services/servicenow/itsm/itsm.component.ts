...
export class ItsmComponent implements OnInit {
  ...
  constructor(private httpClient : HttpClient,private titleService: Title,private metaService: Meta,private location:Location,private renderer: Renderer2,private router: Router) {
    this.titleService.setTitle('ITSM Consulting Services for Seamless IT Operations');
    this.metaService.addTag({ name: 'description', content:"Unlock the potential of your IT operations with our expert ITSM consulting services. Achieve seamless processes and enhanced efficiency for optimal business performance." });
  ...
  blogList(){
    this.httpClient
    .get('https://qbrainx.com/qbrainx-web/v1/Common-Service/byServiceName?serviceName=itsm', {
